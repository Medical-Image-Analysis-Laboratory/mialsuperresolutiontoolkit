...
* The PNG images generated by the stack auto-reordering node and used in the HTML processing report can be found in each subject's ``figures/`` subfolder as:
...
.. topic:: Profiling report
    ...
    if run `--profiling` option flag. It will generate in each subject's ``logs/`` subfolder a ``logs/sub-01_rec-SR_id-1_desc-profiling_log.html`` which reports
