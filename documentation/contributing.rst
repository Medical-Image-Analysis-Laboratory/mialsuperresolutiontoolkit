...
When the PR is merged you will have to make an extra Pull Request where you have to:
    ...
    1. add your entry in the .zenodo.json which is used to generate the DOI for publication (for that you will need an ORCID ID - https://orcid.org/).

    2. update the content of the table in ``documentation/contributors.rst``, which is used by readthedocs, with the new content generated by the bot in the README.
