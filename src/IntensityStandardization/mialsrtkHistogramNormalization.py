...
def main(images, masks, outputs, verbose=False):
    ...
    if len(image_paths) != len(mask_paths):
        raise ValueError(
            "Loading failed: Number of images and masks ",
        ...
        print(
            "Loading passed: Number of images and masks "
            "are equal (# images =",
            str(len(image_paths)),
            "\\ # masks =",
            str(len(mask_paths)),
    ...
    while index < len(image_paths):
        ...
        new_image = nib.Nifti1Image(
            np.reshape(maskedImageMapped, np.array([dimY, dimX, dimZ])),
            image.affine,
            header=image.header,
        )
        if verbose:
...
parser = argparse.ArgumentParser(
    description=("Intensity histogram" "normalization based on" "percentiles")
)
parser.add_argument(
    "-i", "--input", required=True, action="append", help="Input image(s)"
)
parser.add_argument(
    "-m", "--mask", required=True, action="append", help="Input mask(s)"
)
parser.add_argument(
    "-o",
    "--output",
    required=True,
    action="append",
    help="Output normalized image(s)",
)
...
if len(args.input) == 0:
    ...
    print(
        "Usage: %s -i input_image1 -m input_image1_mask " % sys.argv[0],
